...
template<size_t N>
static SaveTableMap::value_type MakeSaveTable(const char *name, const SaveTable::value_type (&t)[N]) {
    SaveTable m;
...
static const SaveTableMap SAVED_TYPES = {
    {MakeSaveTable("Group", SAVED_GROUP)},           {MakeSaveTable("Param", SAVED_PARAM)},
    {MakeSaveTable("Request", SAVED_REQUEST)},       {MakeSaveTable("Entity", SAVED_ENTITY)},
    {MakeSaveTable("Constraint", SAVED_CONSTRAINT)}, {MakeSaveTable("Style", SAVED_STYLE)},
};
...
bool SolveSpaceUI::ReloadLinkedImage(const Platform::Path &saveFile,
    ...
    SS.images[*filename] = std::move(pixmap);
    return true;
}
