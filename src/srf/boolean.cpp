...
static void FindVertsOnCurve(std::vector<SInter> *l, const SCurve *curve, SShell *sh) {
                                    ...
                                    SSurface *srfA, SSurface *srfB) const
{
    ...
    for(++p; p != pts.end(); ++p) {
        ...
        if(!il.empty()) {
            ...
            auto end = std::remove_if(il.begin(), il.end(), [srfA, srfB, this](SInter &pi) {
...
void SSurface::TrimFromEdgeList(SEdgeList *el, bool asUv) {
    ...
    for(;;) {
        ...
        SEdge *se = std::find_if(el->l.begin(), el->l.end(), [](const SEdge &e) {
            return e.tag == 0;
        });
        if(se == el->l.end()) break;
        se->tag = 1;
        stb.start = se->a;
        stb.finish = se->b;
        stb.curve.v = se->auxA;
                                       ...
                                       int dbg_index)
{
    ...
    SPointList choosing = {};
    for(const SEdge &se : orig.l) {
        choosing.IncrementTagFor(se.a);
        choosing.IncrementTagFor(se.b);
    }
    for(const SEdge &se : inter.l) {
        choosing.IncrementTagFor(se.a);
        choosing.IncrementTagFor(se.b);
    }
    for(SPoint &sp : choosing.l) {
        if(sp.tag == 2) {
            sp.tag = 1;
        } else {
            sp.tag = 0;
